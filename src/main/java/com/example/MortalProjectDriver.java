...
// it contains the *main* method, which is the place Java first looks when it runs
//the program
...
// We want to send actual messages to a Mortal object so it will use its
//behaviors
// theMortal.age = 37;
// Send a setter message to theMortal to tell it what its age is
theMortal.setAge(37);
// We use a getter message to ask theMortal what its age is, and we print
//out that value
System.out.println("theMortal age: " + theMortal.getAge());
System.out.println("Law abiding " + theMortal.isLawAbiding());
// We really want to instantiate an object will all its attribute values
//set
...
false, false, true, 1,
"alien", true, 'f');
System.out.println("The otherMortal object has age: " +
otherMortal.getAge() +
...
// When printing an object, we can use randomMortal that is the same as
randomMortal.toString();
...
// Create 18357 Mortal objects and store them in an ArrayList object--we'll
//use a counting loop
// Instantiate an ArrayList object that will hold our Mortal objects
ArrayList<Mortal> mortals = new ArrayList<Mortal>();
int iMortal = 0; // what we'll use to keep track of where we are in the
//loop--loop index
int nMortals = 18357; // how many we want
// start the loop
while (iMortal < nMortals) {
// instantiate a random Mortal object and add it to the list, then
//increment the index
Mortal nextMortal = new Mortal();
mortals.add(nextMortal); // use the add message for ArrayList to add to
//end of list
iMortal = iMortal + 1;
}
// Print out the information for one of the objects in the list using the
//get method
System.out.println("Mortal 2317: " + mortals.get(2317));
System.out.println("and its value is " +
mortals.get(2317).calculateValue());
...
System.out.println("The number of high value mortals is " +
countHighValue);
