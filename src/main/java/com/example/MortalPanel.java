...
// We need MortalPanel to do everything a JPanel does, so we need the extends
public class MortalPanel extends JPanel {
// Add an attribute for the list of Mortal objects that will be provided by
...
// This constructor gets a list of Mortal objects and builds the panel with
...
mortalLabel.setText("Mortal " + mortalIndex + ": " +
displayMortal.getSpecies());
...
add(papyrusButton);
add(timesButton);
// We need to tell Java which radio buttons should be grouped together so
...
// When instantiating a slider, tell the constructor the minimum and
//maximum values and
...
// check each checkbox to see if it's checked and update weight and posture
//if it is
// each JCheckBox has an attribute named selected that is true if the
//checkbox is checked
...
// The actionPerformed method defines what to do when this listener
//receives an
// event
}
// Step 4
...
// behavior that tells the JPanel how to update these attributes of the
//font
updateWeightPosture();
}
}
...
@Override
public void actionPerformed(ActionEvent e) {
// Which button generated the event? Ask the event, get the response,
//and treat it as a JRadioButton
...
// A slider generates an event everytime the user *changes* its value by moving
//the handle
